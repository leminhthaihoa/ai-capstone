...
S3_BUCKET       = "factory-ai-data-capstone"     # update this
THRESH_TABLE    = "sensor-thresholds"
# Every factory sharing a station-type's FEATURE SET — used to know which
# feature list applies to a given factory. Each factory now trains its OWN
# separate model using ONLY its own thresholds (see below) — Station 1 and
# Station 3 both report the same 4 vibration/moisture features, but that's
# a shared SHAPE, not a shared MODEL. Confirmed: their independently-tuned
# thresholds are different enough (in some cases opposite: one station
# tight on vibration_1 and loose on vibration_2, the other the reverse)
# that even averaging the two together didn't serve either station well —
# only fully separate models, one per factory, actually fixed it.
FACTORIES_BY_TYPE = {
    "vibration_moisture": ["factory1", "factory3"],
    "thermal_pressure":   ["factory2"],
}
...
# ── Model: vibration_moisture-shaped (one call per factory: 1 and 3) ───────
def build_vibration_moisture_dataset(factory):
    th = load_representative_thresholds(factory)
    ...
    real_path = f"real_data_{factory}.csv"
    if os.path.exists(real_path):
        raw = pd.read_csv(real_path)
        normal = filter_real_normal_rows(raw, {
            "vibration_1": n_vib1, "vibration_2": n_vib2,
...
# ── Model 2: thermal_pressure (Station 2) ───────────────────────────────────
# ── Model: thermal_pressure-shaped (one call: factory2 only) ───────────────
def build_thermal_pressure_dataset(factory):
    th = load_representative_thresholds(factory)
...
def train_one(name, features, df, n_normal, n_fault):
    print(f"\n--- Training '{name}' ({len(features)} features: {features}) ---")
    print(f"    Total samples: {len(df)} ({n_normal} normal + {n_fault} fault)")
...
def main():
    print("=" * 60)
    print("  Factory AI Training — one model per FACTORY (not per station-type)")
    print("  (synthetic ranges derived from each factory's own live thresholds)")
    print("=" * 60)

    os.makedirs("model", exist_ok=True)
    models = {}            # factory -> {model, scaler, low, high}
    features_by_factory = {}

    # ── vibration_moisture-shaped stations: factory1 and factory3, SEPARATELY ──
    for factory in FACTORIES_BY_TYPE["vibration_moisture"]:
        df, n, f, ranges = build_vibration_moisture_dataset(factory)
        model, scaler, predict, low, high = train_one(factory, FEATURES_BY_MODEL["vibration_moisture"], df, n, f)
        models[factory] = {"model": model, "scaler": scaler, "low": low, "high": high}
        features_by_factory[factory] = FEATURES_BY_MODEL["vibration_moisture"]

        # Sanity test values come from THIS factory's own derived normal/fault
        # ranges — not arbitrary multipliers, and not another factory's
        # ranges — so these correctly reflect what THIS factory's model was
        # actually trained on.
        nr = ranges["normal"]
        fh = ranges["fault_high"]
        v1n, v2n, m1n, m2n = nr["vibration_1"][0], nr["vibration_2"][0], nr["moisture_1"][0], nr["moisture_2"][0]
        v1f, v2f, m1f, m2f = fh["vibration_1"][0], fh["vibration_2"][0], fh["moisture_1"][0], fh["moisture_2"][0]
        print(f"    Sanity tests ({factory}):")
        tests = [
            ("Normal",              {"vibration_1": v1n, "vibration_2": v2n, "moisture_1": m1n, "moisture_2": m2n}),
            ("Both vibration high", {"vibration_1": v1f, "vibration_2": v2f, "moisture_1": m1n, "moisture_2": m2n}),
            ("One vibration high",  {"vibration_1": v1f, "vibration_2": v2n, "moisture_1": m1n, "moisture_2": m2n}),
    ...
    # ── thermal_pressure-shaped stations: factory2 only ─────────────────────
    for factory in FACTORIES_BY_TYPE["thermal_pressure"]:
        df, n, f, ranges = build_thermal_pressure_dataset(factory)
        model, scaler, predict, low, high = train_one(factory, FEATURES_BY_MODEL["thermal_pressure"], df, n, f)
        models[factory] = {"model": model, "scaler": scaler, "low": low, "high": high}
        features_by_factory[factory] = FEATURES_BY_MODEL["thermal_pressure"]

        nr = ranges["normal"]
        fh = ranges["fault_high"]
        fl = ranges["fault_low"]
        t1n, t2n, t3n = nr["temperature_1"][0], nr["temperature_2"][0], nr["temperature_3"][0]
        p1n, p2n, usn = nr["pressure_1"][0], nr["pressure_2"][0], nr["ultrasonic"][0]
        t1f, t2f, t3f = fh["temperature_1"][0], fh["temperature_2"][0], fh["temperature_3"][0]
        p1f, p2f      = fh["pressure_1"][0], fh["pressure_2"][0]
        us_high_f     = fh["ultrasonic"][0]
        us_low_f      = fl["ultrasonic"][0]
        print(f"    Sanity tests ({factory}):")
        print("    (Note: 'Storage overfull'/'Storage low/empty' below are a single-feature")
        print("     deviation (ultrasonic alone) among 6 total features, vs. thermal's 3 and")
        print("     pressure's 2 — IsolationForest inherently isolates multi-feature anomalies")
    ...
    # ── Save and package ALL factories' models, keyed by FACTORY name ──────
    print(f"\nSaving and packaging {len(models)} per-factory models...")
    calibration = {}
    tar_members = []
    for factory, m in models.items():
        joblib.dump(m["model"],  f"model/isolation_forest_{factory}.joblib")
        joblib.dump(m["scaler"], f"model/scaler_{factory}.joblib")
        calibration[factory] = (m["low"], m["high"])
        tar_members += [f"isolation_forest_{factory}.joblib", f"scaler_{factory}.joblib"]

    joblib.dump(features_by_factory, "model/features_by_model.joblib")
    joblib.dump(calibration, "model/score_calibration.joblib")

    with tarfile.open("model.tar.gz", "w:gz") as tar:
        for name in tar_members:
            tar.add(f"model/{name}", arcname=name)
        tar.add("model/features_by_model.joblib", arcname="features_by_model.joblib")
        tar.add("model/score_calibration.joblib", arcname="score_calibration.joblib")
